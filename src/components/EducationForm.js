...
import { getEducationAction, studentEducationInfoAction } from '../action/studenteducation';
...
export const EducationForm = (
    ...
    const getStudentEducation = React.useCallback(() => dispatch(getEducationAction(token, studentInfo.ID)), [dispatch, token, studentInfo]);

    React.useEffect(() => {
        getStudentEducation();
    }, [getStudentEducation]);

    console.log("state education loading ",currentEducation[0])

    const formik = useFormik({
        initialValues:{
            iname: currentEducation[0]?.institute_name || "",
            iaddress: currentEducation[0]?.institute_address || "",
            gpa: currentEducation[0]?.gpa || "",
            grade: currentEducation[0]?.grade || "",
            passedYear: currentEducation[0]?.completed_year || "",
            remarks: currentEducation[0]?.remarks || "",

            cname: currentEducation[1]?.institute_name || "",
            caddress: currentEducation[1]?.institute_address || "",
            cgpa: currentEducation[1]?.gpa || "",
            cgrade: currentEducation[1]?.grade || "",
            cpassedYear: currentEducation[1]?.completed_year || "",
            cremarks: currentEducation[1]?.remarks || "",
        },
